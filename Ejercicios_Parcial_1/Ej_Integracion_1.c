...
int main(void){
	...
	while(1){
		if(flagDigito){
			...
			digito++;
			/* La cuenta es de 0 a 9 */
				if(digito > 9){
					digito = 0;
				}
			/* Saca el valor por los pines P2.0 a P2.9*/
...
void configSysTick(uint32_t ticks){
	SysTick-> CTRL &= ~(3<<0);					// Desactiva el SysTick, y la interrupción.
 ...
 * Si se lee un 0 lógico en P0.1 antes de contar las interrupciones
 * - Se descativa SysTick
 * - Se configura de nuevo la int. en P0.1
 * - No se saca el dígito por el P2.0 a P2.7
...
void SysTick_Handler(void){
	...
	if(LPC_GPIO0->FIOPIN & (1<<1)){
		...
		else flagDigito = 1;     // Activa el flag para sacar el digito por puerto.
	}
	SysTick-> CTRL &= ~(3<<0);			// Desactiva el SysTick, y la interrupción.	// Desactiva el SysTick.
